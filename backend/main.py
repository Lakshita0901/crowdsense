...
def check_rate_limit(
    client_ip: str, limit: int = 15, window: int = 60
) -> bool:
    """
    ...
    timestamps = [
        t for t in _rate_limit_records[client_ip] if now - t < window
    ]
...
async def startup() -> None:
    ...
    try:
        ...
        print(f"Error: Density file not found at {DENSITY_FILE}")
        _density = {}
    except json.JSONDecodeError as err:
        print(f"Error decoding JSON in density file: {err}")
        _density = {}
    ...
    if INDEX_FILE.exists() and META_FILE.exists():
        try:
            with open(META_FILE, "rb") as fh:
                ...
                pickle.UnpicklingError, OSError) as err:
            print(f"   Warning: Failed to load FAISS meta file: {err}")
        ...
        if faiss is not None:
            try:
                ...
                print(f"   Warning: Failed to load FAISS index: {err}")
                _faiss_index = None
    else:
    ...
    if _gemini_llm:
        ...
        print("   WARNING: No GOOGLE_API_KEY -- "
              "fan chat will use FAISS-only fallback.")
...
class FanChatRequest(BaseModel):
    ...
    geolocation: Optional[dict] = None  # {"lat": float, "lng": float}
...
class FanChatResponse(BaseModel):
    ...
    llm_used: bool
    routing_alert: bool = False  # True when density reroute applied
...
def health() -> dict:
    ...
    Returns:
        dict: A mapping of system statuses, FAISS vectors loaded,
              and current time.
    """
...
def get_floorplan() -> dict:
    ...
    if not _floorplan:
        raise HTTPException(
            status_code=503, detail="Floor-plan data not loaded."
        )
    return _floorplan
...
def get_density() -> dict:
    ...
    Returns:
        dict: Per-gate counts, queue wait times, and stadium totals.
...
def _simulate_gate_drift(gate: dict, now: str) -> int:
    """
    Simulate one density drift step for a single gate.

    Args:
        gate (dict): The gate state dictionary to update.
        now (str): ISO-formatted string of the current time.

    Returns:
        int: The simulated new crowd count for this gate.
    """
    cap = gate["capacity"]
...
def update_density() -> dict:
    ...
    Raises:
        HTTPException: If density dataset is not loaded.
    """
    if not _density:
        raise HTTPException(status_code=503, detail="Density data not loaded.")
    ...
    for gate in _density.get("gates", []):
        total += _simulate_gate_drift(gate, now)
    ...
    _density["stadium_totals"] = {
        ...
        "gates_at_critical": sum(
            1 for g in _density["gates"] if g["status"] == "critical"
        ),
        "gates_at_high": sum(
            1 for g in _density["gates"] if g["status"] == "high"
        ),
    }
    return _density
...
def _nearest_gates(gate_id: str) -> str:
    ...
    Returns:
        str: Description of the nearest alternative gates
             (e.g., 'Gate H or Gate B').
    """
...
def _ask_fallback(req: AskRequest) -> AskResponse:
    """
    Keyword-based retrieval synthesis when FAISS index is not initialized.

    Args:
        req (AskRequest): Request containing query and optional top_k.

    Returns:
        AskResponse: Synthesis of matching text lines and
    ...
    k_val = req.top_k or TOP_K
    for score, idx, chunk, meta in ranked[:k_val]:
        sources.append({
            "chunk_index": int(idx),
            "score": float(score),
            "type": meta.get("type"),
...
def _ask_faiss(req: AskRequest) -> AskResponse:
    """
    FAISS-based vector similarity search and retrieval synthesis.

    Args:
        req (AskRequest): Request containing query and optional top_k.

    Returns:
        AskResponse: Synthesis of similar chunks and source metadata.
    """
    try:
        ...
        return AskResponse(
            query=req.query,
            answer=f"Embedding unavailable: {_embed_err}",
            sources=[]
        )

    k_val = min(req.top_k or TOP_K, _faiss_index.ntotal)
    try:
        distances, indices = _faiss_index.search(q_vec, k_val)
    except Exception as search_err:
        return AskResponse(
            query=req.query,
            answer=f"FAISS search failed: {search_err}",
            sources=[]
        )
    ...
    for dist, idx in zip(distances[0], indices[0]):
        if idx < 0:
            continue
        if idx >= len(metadata) or idx >= len(chunks):
            continue
        sources.append({
            "chunk_index": int(idx),
            "score": float(dist),
...
def ask(req: AskRequest) -> AskResponse:
    ...
    Args:
        req (AskRequest): Request payload containing user query
                          and optional top_k.
    ...
    if _faiss_index is None:
        return _ask_fallback(req)
    return _ask_faiss(req)
...
def _synthesise(query: str, context: str) -> str:
    ...
    top = [line for score, line in matched[:3] if score > 0]
    if top:
        return ("Based on the CrowdSense AI knowledge base:\n\n"
                + "\n\n".join(f"• {line}" for line in top))
    return ("I found relevant information but couldn't pinpoint an exact "
            "answer:\n\n" + "\n\n".join(f"• {line}" for line in lines[:3]))


@app.post("/api/fan/chat", response_model=FanChatResponse, tags=["Fan Chat"])
async def fan_chat_endpoint(
    req: FanChatRequest, request: Request
) -> FanChatResponse:
    """
    Multilingual fan assistant.
    Retrieves stadium context via FAISS and generates a native-language
    response using Gemini 2.5 Flash.
    Falls back to FAISS-only synthesis if GOOGLE_API_KEY is not set.

    Args:
        req (FanChatRequest): The fan's query, preferred language, current
                              ticket location info, and top_k context limit.
        request (Request): The incoming FastAPI request instance to extract IP
                           for rate limiting.

    Returns:
        FanChatResponse: Generative AI answer along with explainability why
                         explanation and retrieval sources.

    Raises:
        HTTPException: If query is empty, too long, contains malicious
                       content, or if client is rate-limited.
    """
    client_ip = "unknown"
    ...
    malicious_patterns = [
        "<script", "javascript:", "union select", "drop table", "or 1=1"
    ]
    if any(pat in query_str.lower() for pat in malicious_patterns):
        raise HTTPException(
            status_code=400,
            detail="Query contains forbidden patterns or malicious content."
        )

    # Pass the full density dict (not just a pre-flattened summary) so fan_chat
    # can cross-reference section->gate->density and route appropriately.
...
def get_match_clock() -> dict:
    ...
    Returns:
        dict: The match-clock context dictionary and
              gate-table context metrics.
    """
    from fan_chat import (  # noqa: PLC0415
